...
#type: nothing <-- just right
    sql: EXTRACT({% parameter pop.within_period_type %},DATE_ADD({% date_end pop.date_filter %}, INTERVAL0 - ${within_periods.n} {% parameter pop.within_period_type %}));;
  }
...
#### ------------ VIEWS TO EDIT  ------------ ###
  ...
  view: pop_order_items_created {
    ...
    sql_table_name: (
          SELECT
              EXTRACT({% parameter pop.within_period_type %} FROM order_items.created_at) as join_date,
              COUNT(*) as agg_1,
...
## We make one view per date and join them all to the model
    ...
    view: pop_order_items_delivered {
      ...
      sql_table_name: (SELECT
            EXTRACT({% parameter pop.within_period_type %} FROM order_items.shipped_at) as join_date,
            COUNT(*) as agg_1,
            SUM(order_items.sale_price) as agg_2
            FROM order_items
