...
include: "/views/**/*.view"
include: "/pop_mysql_views/*.view.lkml"

datagroup: pop_mysql_default_datagroup {
   sql_trigger: SELECT HOUR(CURTIME());;
  max_cache_age: "1 hour"
}

persist_with: pop_mysql_default_datagroup

explore: order_items_original {}
explore: order_items {
...
explore: pop_simple {
  ...
  always_filter: {
    filters: [choose_breakdown: " ", choose_comparison: " "]
  }
}
...
# No editing needed
  join: within_periods {
    ...
    sql_on: ${within_periods.n}
            <= DATEDIFF({% date_end pop.date_filter %}, {% date_start pop.date_filter %})
...
# Rename (& optionally repeat) below join to match your pop view(s)
  join: pop_order_items_created {
    ...
    sql_on: pop_order_items_created.join_date = extract({% parameter pop.within_period_type %} FROM
                      DATE_ADD(
